...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], dropout=self.dropout)
...
def load(name, pretrained=False, progress=False, **kwargs):
    """load any kind of resnet
    Args:
        name: string name of the base network
    return:
        model (str): name of the model
        pretrained (bool): if the model is pretrained
        progress (bool): show progress while loading
        **kwargs
    """
    name = name.lower()
    if name == "resnet18":
        return resnet18(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "resnet34":
        return resnet34(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "resnet50":
        return resnet50(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "resnet101":
        return resnet101(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "resnet152":
        return resnet152(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "resnext50_32x4d":
        return resnext50_32x4d(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "resnext101_32x8d":
        return resnext101_32x8d(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "wide_resnet50_2":
        return wide_resnet50_2(pretrained=pretrained, progress=progress, **kwargs)

    elif name == "wide_resnet101_2":
        return wide_resnet101_2(pretrained=pretrained, progress=progress, **kwargs)
