# Taken from https://github.com/ElementAI/baal/blob/a9cc0034c40d0541234a3c27ff5ccbd97278bcb3/baal/active/heuristics/heuristics.py#L261
...
class AbstractHeuristic:
    ...
    def get_uncertainties(self, predictions):
        ...
        if not np.all(np.isfinite(scores)):
            ...
            warnings.warn(
                f"Invalid value in the score, will be put to {fixed}", UserWarning
            )
...
class BALD(AbstractHeuristic):
    ...
    def compute_score(self, predictions):
        ...
        entropy_expected_p = -np.sum(
            xlogy(expected_p, expected_p), axis=1
        )  # [batch size, ...]
...
class BatchBALD(BALD):
    ...
    def _sample_from_history(self, probs, num_draw=1000):
        ...
        probs = gather_expand(
            expanded_probs_N_K_K_C, dim=-1, index=expanded_choices_N_K_K_S
        )
    ...
    def compute_score(self, predictions):
        ...
        for step in range(self.num_samples):
            ...
            selected = self._sample_from_history(
                predictions[history], num_draw=self.num_draw
            )
...
class CombineHeuristics(AbstractHeuristic):
    ...
    def __init__(
        self, heuristics: List, weights: List, reduction="mean", shuffle_prop=0.0
    ):
        super(CombineHeuristics, self).__init__(
            reduction=reduction, shuffle_prop=shuffle_prop
        )
        ...
        if all(item is False for item in reversed):
            ...
            raise Exception(
                "heuristics should have the same value for `revesed` parameter"
            )
    ...
    def get_uncertainties(self, predictions):
        ...
        for ind, prediction in enumerate(predictions):
            if isinstance(predictions[0], types.GeneratorType):
                results.append(
                    self.composed_heuristic[ind].get_uncertainties_generator(prediction)
                )
            else:
                results.append(
                    self.composed_heuristic[ind].get_uncertainties(prediction)
                )
        return results
